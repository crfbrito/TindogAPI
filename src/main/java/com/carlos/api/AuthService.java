...
import com.carlos.api.JSONService;
...
public class AuthService extends JSONService {
	...
	public Response authenticateCredentials(@HeaderParam("email") String email,
		...
		try {
			user = UserEM.getInstance().getUserByEmailAndPass(email, password);
		...
		claims.setIssuer("tindog.com"); // who creates the token and signs it
